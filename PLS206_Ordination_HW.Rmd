...
#### 4. Visualize these samples using non-metric multidimensional scaling (NMDS). Use the Bray-Curtis (`bray`) dissimilarity metric. Plot this output using `ordiplot()`. Add environmental vectors with `envfit()`
...
1. Which variables appear significantly correlated with the unconstrained ordination?  

2. What is the default behavior of the `envfit` plot for different variable types? Why do some points show arrows, while others are text? Check `?envfit` for help.

__Using the plot below, answer:__

3. Compare and interpret the following plots that use different dissimilarity indices. What do repeated ordinations with different metrics tell you about the different properties of a community?
...
1. Perform a redundancy analysis (`rda`) of the dune community using all environmental predictors.

```{r}
myrda <- rda(dune ~ ., dune.env)
```
...
5. Plot the ordination.

```{r}
ordifinal <- ordistep(myrda)
```

6. Plot this final ordination and interpret your plot
